...
import { ArrowRightOnRectangleIcon, ChatBubbleLeftRightIcon, SignalIcon, MoonIcon, SunIcon } from "@heroicons/react/24/outline";
...
const Navbar = () => {
  ...
  return (
    <header className="relative z-50 flex h-[4.5rem] shrink-0 items-center justify-between border-b border-white/70 bg-white/75 px-4 dark:border-slate-800/80 dark:bg-slate-950/75 shadow-[0_8px_30px_rgba(38,48,82,0.06)] backdrop-blur-xl sm:px-7">
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <button onClick={toggleTheme} className="rounded-xl border border-slate-200 bg-white p-2 text-slate-500 shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-indigo-500/50 dark:hover:bg-slate-800" title={`Switch to ${theme === "dark" ? "light" : "dark"} mode`} aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}>{theme === "dark" ? <SunIcon className="h-4 w-4" /> : <MoonIcon className="h-4 w-4" />}</button>
