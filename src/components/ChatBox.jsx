...
const ChatBox = ({ chat, currentUser, onChatDeleted, onGoBack }) => {
    ...
    const editMessage = (message) => {
        ...
        if (content && content !== message.content) WebSocketService.editMessage(chat.chatId, message.id, content);
    };

    const deleteMessage = (message) => {
        if (window.confirm("Delete this message for everyone?")) WebSocketService.deleteMessage(chat.chatId, message.id);
    };
